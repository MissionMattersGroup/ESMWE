   ...
   $(document).ready(function(){
        $.ajax({
        type: "GET",
        url: "/aspx/RiskAssessmentSetupForSections/showTheFieldsOnLoad",
    ...
    function getTheRecordsFromObjConfig(){
        ...
        success: function(comment) {
            if(comment !== false){
                ...
                var selRiskAssessment = $('#'+'multiselect_rightList'+dynamicId+'');
                ...
                for(k=1;k<=counterForOnLoadValuesForSection;k++){
                    ...
                    && comment[k-1]!== undefined ){
                        var selRiskAssessment = $('#'+'multiselect_rightList'+k+'');
                        ...
                        if(comment[k-1].Fields_for_Section__c !== null){
                            riskAssessmentFields  = comment[k-1].Fields_for_Section__c.split(',');
                            $.each(riskAssessmentFields,function(i){
                                    if(riskAssessmentFields[i] !== "" && riskAssessmentFields[i] !== null){
                                        selRiskAssessment.append('<option class="optionsClass" value="' + riskAssessmentFields[i] + '">' + riskAssessmentFields[i] + '</option>');   
                                    }
    ...
    */
    function getFieldDetails()
    {
            for(f=0;f<=counterValForForm;f++){
                ...
                getTheEntityId("Force__Risk_Assessment__c",sel,riskAssessmentFields);
            }
    }
    ...
    function removeRows(){
        ...
        if(counterValForForm>2){
            ...
            if(idOfTheRecordsForDel!== null && idOfTheRecordsForDel!== "" && idOfTheRecordsForDel !== undefined){
                REST2.exec({ 
                    url:"/aspx/RiskAssessmentSetupForSections/deletePrevRecords" , 
    ...
    */
    function addRows(){
        showDialog();
            ...
            frm+= '<label id="sequenceNoId">Fields For Risk Assessment</label><br/>';
            ...
            frm+= '<b>Collapse Section</b> <input type="checkbox" id=collapseId'+counterValForForm+' name=collapse'+counterValForForm+' value="collapse" checked><br>';
            frm+= '</div>';
    ...
    */
    function Save(btnName){
        ...
        updateTheRiskAssessmentSection(btnName,servicePlanModelArray);
    }
    ...
    */
    function saveValue(i){
            ...
            var riskAssessmentSection = { Title_Of_Section__c:sectionTitle,Sequence__c :sequenceNumber, 
    ...
    */
    function updateTheRiskAssessmentSection(btnName,servicePlanModelArray){
        showDialog();
        REST2.exec({ 
                    url:"/aspx/RiskAssessmentSetupForSections/updateRiskAssessmentSectionInfo" , 
                        ...
                        { 
                             ...
                             if (REST2.isSuccess(context)) {
                                ...
                                if(btnName === 'Save'){
                                    ...
                                    window.location.assign("https://eastersealsmidwest.magentrixcloud.com/aspx/RiskAssessmentSetupForSections");
                                }
    ...
    */
    function getTheValueCheck(valOfRadioBtn){
     ...
     /*
    *@Decription: Function to redirect to the previous page
    */
    function redirectToPrevious(){
        showDialog();
        window.location =  "https://eastersealsmidwest.magentrixcloud.com/aspx/ServicePlanSetupForSections";
        hideDialog();
    }